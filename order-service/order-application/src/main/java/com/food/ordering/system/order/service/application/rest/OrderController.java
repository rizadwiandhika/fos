...
// Forward requests to domain layer
@Slf4j
...
public class OrderController {
	...
	public ResponseEntity<TrackOrderResponse> getOrderByTrackingId(@PathVariable UUID trackingId) {
		TrackOrderResponse response = orderApplicationService
				.trackOrder(TrackOrderQuery.builder().orderTrackingId(trackingId).build());

		log.info("Returning order status with trakcing id: {}", response.getOrderTrackingId());
		return ResponseEntity.ok(response);
	}
